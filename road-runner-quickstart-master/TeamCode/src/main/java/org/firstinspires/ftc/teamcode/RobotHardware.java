...
public abstract class RobotHardware extends RobotBase {
    ...
    protected DcMotorEx spool;

    public static double armP = 12;
    public static double armI = 0.2;
    ...
    public void initRobot (RobotRunType robotRunType){
        ...
        spool = hardwareMap.get(DcMotorEx.class, "elv");

        spool.setTargetPosition(0);
        spool.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        spool.setPIDCoefficients(DcMotor.RunMode.RUN_TO_POSITION, new PIDCoefficients(armP, armI, armD));
    ...
    /**
     * raises the arm to a set tower level
     *
     * @param level skyscraper level to raise to
     */
    protected void raiseArm(int level){

        //calculate needed variables
        final double ppr = ORBITAL20_PPR;
        final double spoolDiameter = 1.98;
        final double spoolCirc = Math.PI * spoolDiameter;
        final double ppi = ppr/spoolCirc;

        //set encoder target to correct level
        int target = (int) (ppi*4*level + 2.5*ppi);

        //special positions for fully down and collection
        if(level == 0){
            target = 0;
        } else if (level == -1){
            target = (int)(2.5*ppi);
        ...
        //apply target position to spool
